...
if (isset($_GET["action"])) {
  if ($_GET["action"] == "delete") {
    ...
    $query = mysqli_query($koneksi, "INSERT INTO transaksi (tgl_beli) values ('" . date("Y-m-d") . "')");
    $id = mysqli_insert_id($koneksi);

    foreach ($_SESSION["cart"] as $key => $value) {
      $id_produk = $value['id_produk'];
      $qty = $value['item_quantity'];
      $sql = "INSERT INTO detail (id_produk,qty) values ('$id_produk', '$qty')";
      $res = mysqli_query($koneksi, $sql);
    }
    unset($_SESSION["cart"]);
    echo '<script>alert("Terima kasih sudah berbelanja")</script>';
    echo '<script>window.location="keranjang.php?id="id_transaksi" "</script>';
  }
}
...
<head>
  ...
  <link href="script src= https://cdn.jsdelivr.net/npm/bootstrap@5.3.1/dist/js/bootstrap.bundle.min.js"
      ...
      <tbody class="table-group-divider">
        ...
        if (!empty($_SESSION["cart"])) {
          ...
          <tr>
            ...
            <td> <a href="keranjang.php?action=beli" class="btn btn-outline-success" type="submit"
