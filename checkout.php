...
<?php
  ...
  if(isset($_POST['order'])){
    $id_pengguna = $_GET['id'];
    $total = $_POST['harga'];
    $sql1= "INSERT INTO transaksi (tgl_beli, id_pengguna,total_harga) values ('$id','$id_produk','$total')";
   
    $qi = mysqli_query($koneksi,$sql1);
    // $_SESSION['idtran'] = $id;
    $last = mysqli_insert_id($koneksi);

    if ($qi){
    // "<script>alert('Berhasil Belanja') window.location = 'cetak.php';
    // </script>";
    header("Location: cetak.php?i=" . $last);
  }
}
?>